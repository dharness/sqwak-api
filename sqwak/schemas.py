...
class MlAppSchema(ma.SQLAlchemyAutoSchema):
  ...
  published_model = fields.Raw()

  class Meta:
    model = MlApp
    exclude = ('working_model', 'published_model')
...
class MlClassSchema(ma.SQLAlchemyAutoSchema):
  class Meta:
    model = MlClass
...
class AudioSampleSchema(ma.SQLAlchemyAutoSchema):
  class Meta:
    model = AudioSample
